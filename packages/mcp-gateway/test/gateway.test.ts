...
describe('MCP Gateway Server Core (In-Memory)', () => {
  ...
  it('should list all registered stub tools over in-memory transport', async () => {
    ...
    expect(names).toContain('get_symbol_definitions_from_file');
  ...
  it('should run get_symbol_definitions_from_file and return the squeezed result', async () => {
    const server = createServer();
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  ...
  it('should run get_symbol_definitions_from_file with filePath and auto-detect language', async () => {
    const server = createServer();
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
