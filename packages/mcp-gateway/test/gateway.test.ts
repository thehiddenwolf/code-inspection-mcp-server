...
describe('MCP Gateway Server Core (In-Memory)', () => {
  ...
  it('should list all registered stub tools over in-memory transport', async () => {
    ...
    expect(names).toContain('token_squeezer.squeeze');
    expect(names).toContain('architecture_shepherd.check');

    // Clean up
    await Promise.all([client.close(), server.close()]);
  });

  it('should run token_squeezer.squeeze and return the squeezed result', async () => {
    const server = createServer();
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
  ...
  it('should run token_squeezer.squeeze with filePath and auto-detect language', async () => {
    const server = createServer();
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
